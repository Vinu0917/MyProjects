...
class StudyTrackerApp {
  ...
  async signup(username, password) {
    ...
    this.currentUser = userData;
    this.studyLogs = []; // Fresh start for new user
    this.saveUserData();
  ...
  logout() {
    this.currentUser = null;
    this.studyLogs = [];
    localStorage.removeItem('studyTracker_user');
    localStorage.removeItem('studyTracker_logs');
   ...
   */
  addStudySession(subject, date, startTime, endTime) {
    ...
    this.studyLogs.push(session);
    this.saveUserData();
    this.showAlert('Study session added successfully!', 'success');
    this.updateDashboard();
    return session;
  }
  ...
  drawLineChart(ctx, canvas, data) {
    ...
    const barWidth = chartWidth / subjects.length * 0.8;
    const barSpacing = chartWidth / subjects.length * 0.2;
  ...
  setupModalEventListeners() {
    ...
    if (showSignup) {
      showSignup.addEventListener('click', () => this.showModal('signup'));
    }
    
    if (showLogin) {
      showLogin.addEventListener('click', () => this.showModal('login'));
    }
  ...
  setupFormEventListeners() {
    ...
    if (studyForm) {
      studyForm.addEventListener('submit', (e) => {
        ...
        if (this.addStudySession(subject, date, startTime, endTime)) {
          studyForm.reset();
    ...
    if (startTimeInput) {
      startTimeInput.addEventListener('change', () => this.updateDurationDisplay());
    }
    
    if (endTimeInput) {
      endTimeInput.addEventListener('change', () => this.updateDurationDisplay());
    }
  }
